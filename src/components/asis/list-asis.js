...
const List_asis = () => {
	...
	const columnsPDF = [
		{title: "ID", field: "id"},
		{title: "Cedula", field: "ced"},
		{title: "Fecha", field: "fec"},
	]

	const downloadPdf=()=>{
		const doc=new jsPDF()
		doc.text("Guaytambos Fit Asistencias",20,10)
		doc.autoTable({
			theme: "grid",
			columns:columnsPDF.map(col => ({ ...col, dataKey: col.field })),
			body:data,
		})
		doc.save('gfit_asistencias.pdf')
					...
					<Col sm="12">
						<Card>
							...
							<CardBody>
								<Col xl="7 sm-20 row">
									<Card className="col" >
										<Link to="/asis/create-asis" className="btn btn-primary">
											Nuevo Asistencia
										</Link>
									</Card>
									<Card className="col">
